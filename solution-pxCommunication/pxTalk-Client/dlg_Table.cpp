...
static const char  *_rect_name  = "client-table.rect";
...
static bool _Test( wsClient *p_client )
{
	...
	for( int i = 0; !_b_stop && i < _TEST_MAX_COUNT; i++ )
	{
		...
		if( !p_client->Send( bf, _TEST_BUFFERSIZE, true, &_b_stop ) ){ _ring_msg->txt_Push( "Send: ERR.[ %d ]", i ); return false; }
		t    = GetTickCount();
		recs[ i ].send_tick = t - tick;
		tick = t;

		// Receive..
		recv_size = p_client->Recv( bf,  _TEST_BUFFERSIZE, true, &_b_stop );
		t    = GetTickCount();
		recs[ i ].recv_tick = t - tick;
		tick = t;

		if( recv_size < 0 ){ _ring_msg->txt_Push( "Recv: ERR.[ %d ]", i ); return false; }
		if( recv_size == _TEST_BUFFERSIZE ) memcpy( &recs[ i ].recv_code, &bf[ _TEST_BUFFERSIZE - 4 ], sizeof(int) );
		else                                recs[ i ].recv_code = -1;

		if( _TEST_SLEEP ) Sleep( _TEST_SLEEP );
	}

	char path_dst[ MAX_PATH ];
	sprintf( path_dst, "%s\\%s", g_dir_profile, "test-client.txt" );
...
static unsigned __stdcall _Thread_Table_Client( void *p )
{
	wsClient *p_client = (wsClient*)p;

	ConnectTo cnct;
	int       err ;

	_ring_msg->txt_Push( "Connecting.." );

	if( !p_client->Connect( cnct.ip_address, cnct.port_no, &err ) )
	{
		_ring_msg->txt_Push( "Connection: Error." );
		goto End;
	}

	_ring_msg->txt_Push( "Connection: OK." );
	_b_connected = true;

//	if( !_Test( p_client ) ) goto End;
	...
	while( !_b_stop )
	{
		...
		{
		case _NFUNC_Text:
			if( !p_client->Send( _order.text, strlen( _order.text ) + 1 , true, &_b_stop ) )
			{
			...
			break;
		default:
			if( !p_client->Send( "", 0, true, &_b_stop ) ){ _ring_msg->txt_Push( "send: ERR." ); goto End; }
			break;
		}

		int recv_size = p_client->Recv( buf, BUFFERSIZE_TEXT, true, &_b_stop );
...
static void _WM_TIMER( HWND hDlg, wsClient *p_cilent  )
{
	_Timer_Kill( hDlg );
	...
	{
		char text[ BUFFERSIZE_TEXT + 1 ];
		while( _ring_msg->txt_Pop( text ) ) _AddTalkWindow( hDlg, "sever", text, _b_connected ? _LAST_MSG_OTHER : _LAST_MSG_NONE );
...
static void _IDC_BTN_SEND( HWND hDlg, wsClient *p_client )
{
...
static wsClient *_p_client = NULL;

static void _WM_INITDIALOG( HWND hDlg, LPARAM l )
{
	_p_client = (wsClient*)l;
...

	{
		...
		ConnectTo cnct;
		sprintf( title, "%s:%d", cnct.ip_address, cnct.port_no );
	...
	_h_thrd     = (HANDLE)_beginthreadex( NULL, 0, _Thread_Table_Client, (LPVOID)_p_client, 0, &id );

//	_Thread_Table_Client( _p_client );
	_Timer_Set( hDlg );
}

static void _WM_CLOSE( HWND hDlg, wsClient *p_client )
{
	_b_stop = true;
...
BOOL CALLBACK dlg_Table( HWND hDlg, UINT msg, WPARAM w, LPARAM l )
{
	...
	case WM_CLOSE     : _WM_CLOSE     ( hDlg, _p_client ); break;
	case WM_TIMER     : _WM_TIMER     ( hDlg, _p_client ); break;

	case WM_COMMAND   :

		switch( LOWORD(w) )
		{
		case IDCANCEL    : _WM_CLOSE    ( hDlg, _p_client ); break;
		case IDC_BTN_SEND: _IDC_BTN_SEND( hDlg, _p_client ); break;
